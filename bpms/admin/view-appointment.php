...
if(isset($_POST['submit']))
  {
   ...
   $query=mysqli_query($con, "update  tblappointment set Remark='$remark',Status='$status',appointedemployee='$employee' where ID='$cid'");
    if ($query) {
...
<tr>
  ...
  <tr>
    ...
    <td><?php  echo $row['appointedemployee'];?></td>
  </tr>
<tr>
    <th>Status</th>
    <td> <?php  
...
<tr>
    ...
    <td>
        <?php 
        $employee=mysqli_query($con,"select * from tblemployees");
        ?>
        <select name="employee" class="form-control wd-450" required="true">
            <option value="" selected>Select an Employee</option> <!-- Added default option -->
            <?php 
            while ($employeelist=mysqli_fetch_array($employee)) {
                ?>
                <option value="<?php echo $employeelist['name']; ?>"><?php echo $employeelist['name']; ?></option>
                <?php 
            }
            ?>
        </select>
